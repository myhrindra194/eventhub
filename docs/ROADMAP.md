...
| **v2.7** | Explorer : bannière carrousel animée, sections par activité, bascule liste / grille mémorisée, écran « Tous les événements » ; apparence personnalisable (modèle de couleurs, police) dans Profil pour les deux rôles |
| **v2.8** | Explorer sans recherche ni filtres, bannière toujours présente (événements à la une, sinon bannières éditoriales) avec défilement de 5 s, effet de zoom et indicateur de progression ; photo et couverture modifiables depuis Profil avec autorisation caméra / photos ; couleur et police déplacées dans Paramètres |
