...
class AuthRemoteDataSource {
  ...
  Future<UserModel> register({
    ...
    // Le payload correspond exactement aux contraintes de firestore.rules
    // keys().hasOnly(['name', 'email', 'role']) et role in ['participant', 'organizer']
    final profileData = {
      'name': name.trim(),
      'email': email.trim(),
      'role': role,
  ...
  Future<UserModel> login({
    ...
    // S'assurer que le profil Firestore existe pour satisfaire les règles userRole()
    await _ensureProfile(user);
  ...
  Future<void> _ensureProfile(User user) async {
    ...
    // Strictement conforme à request.resource.data.keys().hasOnly(['name', 'email', 'role'])
    await reference.set({
