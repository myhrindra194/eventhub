...
mixin _$EventDto {
 ...
 List<String> get staffIds;/// `tiers.{id} = {name, description, price, capacity, available, order}`.
 Map<String, dynamic> get tiers;/// Omitted when null: the rules only accept a known currency code.
@JsonKey(includeIfNull: false) String? get currency;
...
as Map<String, dynamic>,currency: freezed == currency ? _self.currency : currency // ignore: cast_nullable_to_non_nullable
as String?,
  ));
}
...
/// `tiers.{id} = {name, description, price, capacity, available, order}`.
 final  Map<String, dynamic> _tiers;
/// `tiers.{id} = {name, description, price, capacity, available, order}`.
@override@JsonKey() Map<String, dynamic> get tiers {
  if (_tiers is EqualUnmodifiableMapView) return _tiers;
  // ignore: implicit_dynamic_type
  return EqualUnmodifiableMapView(_tiers);
}
...
@override
int get hashCode {
    return Object.hash(runtimeType,title,description,category,startsAt,location,capacity,availablePlaces,organizerId,organizerName,imageUrl,createdAt,updatedAt,const DeepCollectionEquality().hash(_staffIds),const DeepCollectionEquality().hash(_tiers),currency);
}

@override
String toString() {
    return 'EventDto(title: $title, description: $description, category: $category, startsAt: $startsAt, location: $location, capacity: $capacity, availablePlaces: $availablePlaces, organizerId: $organizerId, organizerName: $organizerName, imageUrl: $imageUrl, createdAt: $createdAt, updatedAt: $updatedAt, staffIds: $staffIds, tiers: $tiers, currency: $currency)';
}
